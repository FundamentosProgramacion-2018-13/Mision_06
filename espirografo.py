...
def dibujarmandala (ventana,r,R,l):
    ...
    while not termina:  # Ciclo principal, MIENTRAS la variabel termina sea False, el ciclo se repite automáticamente
        ...
        for angulo in range(0, vueltasperiodo + 1, 1):
            ...
            x = int (R*((1 - k) * (math.cos(anguloValorRadianes)) + l * k * (math.cos(anguloEcuacion * anguloValorRadianes)))) #ecuación principal en x
            y = int (R*((1 - k) * (math.sin(anguloValorRadianes)) + l * k * (math.sin(anguloEcuacion * anguloValorRadianes)))) #ecuacion principal en y
